...
# 拼接网址
url = 'https://list.mogu.com/search/goods?q=粽子&ptp=31.nXjSr.0.0.E6isLMAd'
...
for page in range(33):
    ...
    # 5. 提取数据
    divs = driver.find_elements(By.CSS_SELECTOR, '.pin-item-wrap')

    for div in divs:
        print(div)

        title = div.find_element(By.CSS_SELECTOR, '.pin-info-title').text
        price = div.find_element(By.CSS_SELECTOR, '.price-wrap span').text
        ...
        img_url = div.find_element(By.CSS_SELECTOR, '.pin-img img').get_attribute('src')
